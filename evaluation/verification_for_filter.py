...
def resolve_deps_image(
    ...
    Priority:
      ...
      4. Build from Dockerfile (or legacy Dockerfile.deps) in the extracted repo
    ...
    # 3. Build from Dockerfile (or legacy Dockerfile.deps)
    dockerfile = os.path.join(repo_dir, "Dockerfile")
    if not os.path.isfile(dockerfile):
        dockerfile = os.path.join(repo_dir, "Dockerfile.deps")
    if os.path.isfile(dockerfile):
        print(f"         Docker: building from {os.path.basename(dockerfile)} …")
